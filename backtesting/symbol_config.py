...
class SymbolConfig:
    def __init__(self, name: str, exchange: str = "binance", symbol_type: SymbolType = SymbolType.Future,
                 tick_size: float = 0.00001, min_trade_amount: float = 0.00001, max_trade_amount: float = 130,
                 is_active: bool = True):
    ...
    def convert_order_size(self, size) -> float:
        if not size:
            return self.min_order_size
        # A valid size must be the multiple of the ticksize
        return int(size / self.tick_size) * self.tick_size


BTCUSDT = SymbolConfig(name="BTC-USDT")
